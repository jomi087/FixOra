...
import type { jobHistoryInfo } from "@/shared/Types/booking";
...
const BookingHistory = () => {
  ...
  const [jobhistory, setJobHistory] = useState<jobHistoryInfo[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const fetchBookingHistory = async () => {
      try {
        const res = await AuthService.providerJobHistoryApi(currentPage, itemsPerPage);
        setJobHistory(res.data.bookingHistoryData);
